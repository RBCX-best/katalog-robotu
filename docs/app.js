...
function initApp() {
    ...
    // Parse URL query parameter to pre-populate search (e.g. from Hall of Fame link)
    ...
    filteredRobots.forEach((robot, index) => {
        ...
        if (robot.placement && robot.placement !== 'Bez specifického umístění' && robot.placement !== 'Bez umístění') {
            ...
            if (robot.placement.includes('1.')) {
                badgeBgClass = 'bg-amber-500/10 text-amber-300 border-amber-500/30';
                iconColor = 'text-amber-400';
                iconName = 'trophy';
            } else if (robot.placement.includes('2.')) {
                badgeBgClass = 'bg-slate-300/10 text-slate-200 border-slate-350/30';
                iconColor = 'text-slate-300';
                iconName = 'medal';
            } else if (robot.placement.includes('3.')) {
                badgeBgClass = 'bg-amber-700/15 text-amber-600 border-amber-700/30';
                iconColor = 'text-amber-600';
                iconName = 'medal';
            }
            
            placementBadgeHtml = `
                <div class="flex items-center gap-1 px-2 py-0.5 ${badgeBgClass} border rounded-lg text-[10px] font-bold tracking-wide uppercase flex-shrink-0">
        ...
        card.innerHTML = `
            ...
            <div class="p-6 flex flex-col flex-grow">
                ...
                <!-- Robot Name & Placement -->
                <div class="flex items-start justify-between gap-3 mb-2">
                    <h3 class="font-outfit text-xl font-bold text-white group-hover:text-brand-300 transition-colors">
                        ${robot.name}
                    </h3>
                    ${placementBadgeHtml}
                </div>
